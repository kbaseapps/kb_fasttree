...
def get_config():
    ...
    for nameval in config.items(get_service_name() or 'kb_fasttree'):
...
impl_kb_fasttree = kb_fasttree(config)
...
async_run_methods = {}
async_check_methods = {}
async_run_methods['kb_fasttree.run_FastTree_async'] = ['kb_fasttree', 'run_FastTree']
async_check_methods['kb_fasttree.run_FastTree_check'] = ['kb_fasttree', 'run_FastTree']
sync_methods['kb_fasttree.run_FastTree'] = True
...
class Application(object):
    ...
    def __init__(self):
        submod = get_service_name() or 'kb_fasttree'
        ...
        self.rpc_service.add(impl_kb_fasttree.run_FastTree,
                             name='kb_fasttree.run_FastTree',
                             types=[dict])
        self.method_authentication['kb_fasttree.run_FastTree'] = 'required'
                    ...
                    if auth_req != "none":
                        if token is None and auth_req == 'required':
                            ...
                            err.data = "Authentication required for " + \
                                "kb_fasttree but no authentication header was passed"
                            raise err
